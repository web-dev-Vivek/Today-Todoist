...
function addTask(inputId, taskListId, elementsToHide) {
  ...
  circleImage.addEventListener('click', function() {
    checkboxImage.style.display = checkboxImage.style.display === 'none' ? 'block' : 'none';
  });

  // Add an event listener to the delete icon to remove the task item
  ...
  // Add an event listener to the rename icon to edit the task text
  rename.addEventListener('click', function() {
    const currentText = taskText.textContent;
    const inputField = document.createElement('input');
    inputField.type = 'text';
    inputField.value = currentText;
    inputField.style.width = '70%'; // Optional: set input field width to fit in the task div
  ...
  // Add the circle image, checkbox image, text, rename, and delete icons to the task div
